...
pub fn optimize(
    ...
    let attack_element_param =
        ar_calculator::get_attack_element_param(weapon.attack_element_correct_id)?;
    let calc_correct_ids = ar_calculator::get_calc_correct_graph_ids(&weapon.name)?;
    let calc_correct_graphs = ar_calculator::get_calc_correct_graphs(&calc_correct_ids)?;
...
pub fn meet_weapon_requirements(
    ...
    for stat_type in stat_types {
        let stat_value = match stat_type {
            Scaling::Str => &mut stats.strength,
            Scaling::Dex => &mut stats.dexterity,
            Scaling::Int => &mut stats.intelligence,
            Scaling::Fai => &mut stats.faith,
            Scaling::Arc => &mut stats.arcane,
        };
...
mod tests {
    ...
    fn advanced_fallingstar_beastjaw_optimization() {
        ...
        assert_eq!(stats.unspent_levels().unwrap(), 60);
        let weapon =
            weapon::Weapon::from_data("Fallingstar Beast Jaw", 5).expect("failed to create weapon");
